...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div class="text-center justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
      <h1 class="h2">Form Tambah Data Penjualan</h1>
      <form action="/penjualan/create" method="GET" >
        <div class="input-group mb-1 w-50 m-auto">
          <input type="text" class="form-control bg-light border border-dark" placeholder="Cari nama pelanggan..." name="search" value="{{ request('search') }}" autofocus>
  ...
  <div class="row w-100 mt-4">
    <div class="col-md-9 ms-sm-auto col-lg-9 px-md-4">
      ...
      @endif
        <form action="/penjualan" method="POST" class="needs-validation" novalidate>
        @csrf
            <div class="row g-3">
                <div class="col-sm-10">
                ...
                <div class="col-sm-10">
                    ...
                    <select name="user_id" id="user_id" class="form-select" required onmousedown="if(this.options.length>5){this.size=5;}" onchange='this.size=0;' onblur="this.size=0;">
                      @if ($user->count())
                      @foreach ($user as $pelanggan)
                         <option value="{{ $pelanggan->id }}" {{ old('user_id') == $pelanggan->id ? 'selected':''}}>{{ $pelanggan->nama }}</option> 
                      @endforeach
                      @else
                      <option value="none" disabled selected>Pelanggan Tidak Ditemukan</option>
                      @endif
                    </select>
                    </div>
