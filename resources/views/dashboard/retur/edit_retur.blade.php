...
<main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
    <div class="text-center justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
      <h1 class="h2">Form Ubah Data Retur</h1>
      <form action="/retur/{{ $retur->id }}/edit" method="GET" >
  ...
  <div class="row w-100 mt-4">
    <div class="col-md-9 ms-sm-auto col-lg-9 px-md-4">
        <form action="/retur/{{ $retur->id }}" method="POST" class="needs-validation" novalidate>
        @method('put')
        @csrf
            <div class="row g-3">
                <div class="col-sm-10">
                  ...
                  @endif
                    ...
                    <input type="date" name="tanggal_retur" class="form-control @error('tanggal_retur') is-invalid @enderror" id="tanggal_retur" placeholder="" value="{{ old('tanggal_retur' , $retur->tanggal_retur) }}">
                ...
                <div class="col-sm-4">
                   ...
                   <div class="input-group">
                    ...
                    <select name="barang_id" id="barang_id" class="form-select" onmousedown="if(this.options.length>5){this.size=5;}" onchange='this.size=0;' onblur="this.size=0;">
                      @if ($barang->count())
                      @foreach ($barang as $produk)
                         ...
                         @if ($retur->barang_id == $produk->id)
                            <option value="{{ $produk->id }}" data-toggle="tooltip" data-placement="top" title="Stok tersedia {{ $produk->sisa }}" selected>{{ $produk->nama_barang }}</option> 
                         @else
                ...
                <div class="col-sm-3">
                    ...
                    <input type="number" name="jumlah" class="form-control @error('jumlah') is-invalid @enderror" id="jumlah" placeholder="10" value="{{ old('jumlah' , $retur->jumlah) }}">
                    @if (old('jumlah'))
                    @error('jumlah')
                    <div class="invalid-feedback">
                    {{ $message }}
                ...
                <div class="col-sm-3">
                    ...
                    <input type="text" name="keterangan" class="form-control @error('keterangan') is-invalid @enderror" id="keterangan" placeholder="Keterangan pengembalian..." value="{{ old('keterangan' , $retur->keterangan) }}" style="text-transform: capitalize;">
                    @if (old('keterangan'))
                    @error('keterangan')
                    <div class="invalid-feedback">
                    {{ $message }}
            ...
            <button class="col-md-2 btn btn-primary btn-md mt-4" type="submit">Ubah Retur</button>
